...
class AOD_pono_net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=32, kernel_size=1, stride=1, padding=0)
        self.conv2 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, stride=1, padding=1)
        self.conv3 = nn.Conv2d(in_channels=64, out_channels=32, kernel_size=5, stride=1, padding=2)
        self.conv4 = nn.Conv2d(in_channels=96, out_channels=32, kernel_size=7, stride=1, padding=3)
    ...
    def forward(self, x):
        ...
        x3 = F.relu(self.bn1(self.conv3(cat1)))
        x3 = self.pool2(x3)
        cat2 = torch.cat((x2, x3), 1)
        x3 = self.ms(x3, mean1, std1)

        x4 = F.relu(self.bn1(self.conv4(cat2)))
        x4 = self.pool3(x4)
