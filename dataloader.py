...
def populate_train_list(orig_images_path, foggy_images_path):
	...
	train_list = []
	val_list = []
	# print(foggy_images_path)
	image_list_foggy = glob.glob(foggy_images_path + "*.jpg")
	# image_list_foggy = glob.glob(os.path.join(foggy_images_path, "*.jpg"))
 
	# print(image_list_foggy)
	tmp_dict = {}
...
class unfogging_loader(data.Dataset):
	...
	def __init__(self, orig_images_path, foggy_images_path, mode='train'):
		print(orig_images_path,foggy_images_path)
	...
	def __getitem__(self, index):
		...
		data_foggy = data_foggy.resize((480,640), Image.Resampling.LANCZOS)
